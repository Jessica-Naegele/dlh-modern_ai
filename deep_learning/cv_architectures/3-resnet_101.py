...
def build_resnet101(input_shape=(224, 224, 3), num_classes=1000):
    ...
    """
    resnet = keras.layers.Conv2D(filters=64, kernel_size=(7,7), strides=2, padding='same', use_bias=False, 
                            name="conv1")(input)
    resnet = keras.layers.BatchNormalization(name="bn1")(resnet)
    resnet = keras.layers.Activation('relu', name="relu1")(resnet)
    ...
    resnet = keras.layers.GlobalAveragePooling2D(name="GlobalAveragePooling")(resnet)
    outputs = keras.layers.Dense(units=num_classes, activation='softmax', name="Dense")(resnet)
